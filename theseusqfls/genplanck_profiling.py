...
def fit_qfls(energy, pl, guesses=(1.1, 0.01, 1.5, 1.6), batch_size=56, max_iterations=20, step_size=0.5, verbose=True):
    ...
    with torch.no_grad():
        with torch.profiler.profile(
                ...
                schedule=torch.profiler.schedule(
                    ...
                    active=6,
                    repeat=1),
                on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/theseusqfls_cuda2'),
                record_shapes=True,
                profile_memory=True,
                with_stack=True
        ) as prof:
